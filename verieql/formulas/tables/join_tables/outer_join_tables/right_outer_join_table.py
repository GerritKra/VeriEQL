...
from verieql.context import Context
...
from verieql.formulas.expressions.expression_tuple import FExpressionTuple
from verieql.formulas.expressions.predicates import PredicateType
from verieql.formulas.tables.base_table import FBaseTable
from verieql.formulas.tables.join_tables._utils import (
    _analyze_using,
)
from verieql.formulas.tables.join_tables._utils import alias_check
from verieql.formulas.tables.join_tables.outer_join_tables.outer_join_base_table import FOuterJoinBaseTable
from verieql.formulas.tuples.null_tuple import FNullTuple
from verieql.formulas.tuples.outer_join_tuple import FOuterJoinTuple


def _right_outer_product(
        scope,
    ...
    null_tuple = FNullTuple(scope, lhs_table.attributes)
    # for this case where EMPTY_TABLE RIGHT JOIN XXX <=> concate(EMPTY_TABLE, XXX)
    children = {}
    pity_tuples = []

    curr_table = []
    for i, rtuple in enumerate(rhs_table, start=1):
        outer_null_tuple = FOuterJoinTuple(null_tuple, rtuple, name=scope._get_new_tuple_name())
        outer_null_tuple.reverse_attributes()  # reversed attribute for right outer join
...
@register_formula('right_outer_join')
class FRightOuterJoinTable(FOuterJoinBaseTable):
    def __init__(self,
                 scope,
                 tables: Sequence[FBaseTable],
        ...
        name = name or '_RightOuterJoin_'.join(t.name for t in tables)

        # 1) product
        prev_tables, curr_table, null_tuple, self._children, self._pity_tuples = \
            _right_outer_product(scope, tables, name)
        self.left_null_tuple = null_tuple
        self.right_null_tuple = None
        self.is_using = by == 'using'
        self._hidden_attributes = None

        if len(curr_table) == 0:
            super(FRightOuterJoinTable, self).__init__(scope, curr_table, name)
