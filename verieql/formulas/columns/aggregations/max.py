...
from verieql.formulas.columns.aggregations.aggregation import (
    FAggregation,
    CONSTANT_TYPE,
)
...
from verieql.utils import (
    _MAX,
    simplify,
)


@register_formula('agg_max')
class FAggMax(FAggregation):
    def __init__(self, scope, function: str, expression: FBaseExpression, **kwargs):
        super(FAggMax, self).__init__(scope, function, expression, **kwargs)
        ...
        if isinstance(self.EXPR, CONSTANT_TYPE):
            ...
            for y in y_s:
                ...
                value_formulas.append(
                    If(premise, NEG_INF__Int, self._safe_value(curr_expr.VALUE))
                )
                # register bound constraint for MAX(EXPR)
                self.scope.bound_constraints.add(NEG_INF__Int < curr_expr.VALUE)
