...
from verieql.formulas.columns.aggregations.aggregation import FAggregation
...
from verieql.utils import simplify


@register_formula('agg_bool_and')
class FBoolAnd(FAggregation):
    def __init__(self, scope, function: str, expression: FBaseExpression = None, **kwargs):
        # BOOL_AND 1) ignores NULL and 2) cannot handle all NULL
        super(FBoolAnd, self).__init__(scope, function, expression, **kwargs)
            ...
            if self.DISTINCT and idx > 0:
                for i, prev_y in enumerate(y_s[:idx]):
                    ...
                    if i != 0 and group_func is not None:
                        prev_cond.append(group_func(prev_y))  # distinct tuple should also belongs to this group
